...
bool forceTimeToStripe = false;

// push the time (words) to leds array
void timeToStripe()
{
    if (minuteChanged() || isForceTimeToStripe())
    {
        blankscreen();
        ...
        if (minutes < 5)
        {
            word2stripe(word_UHR, sizeof(word_UHR) / sizeof(int));
        }
        FastLED.show();
        forceTimeToStripe = false; // reset forceTimeToStripe in case it was set
    }
}

bool isForceTimeToStripe()
{
    return forceTimeToStripe;
}

void doForceTimeToStripe()
{
    forceTimeToStripe = true;
    debugI("LEDs will be forced to update.");
}
